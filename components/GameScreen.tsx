...
import { GameMode, GameResult, ClassicGameOptions, TimedGameOptions } from '../types';
...
interface GameScreenProps {
  ...
  onGoHome: () => void;
  onGoToShop: () => void;
  classicOptions: ClassicGameOptions | null;
  timedOptions: TimedGameOptions | null;
}
...
const GameScreen: React.FC<GameScreenProps> = ({ mode, onGameEnd, onGoHome, onGoToShop, classicOptions, timedOptions }) => {
  
  const options = mode === 'CLASSIC' ? classicOptions : timedOptions;
  ...
  const {
    ...
    correctAnswersCount,
    bestStreakInGame
  } = useGameLogic(mode, onGameEnd, options);
