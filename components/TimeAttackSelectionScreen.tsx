...
import { Difficulty, TimeAttackGameOptions } from '../types';
import { SoundContext } from '../context/SoundContext';

interface TimeAttackSelectionScreenProps {
  onStartGame: (options: TimeAttackGameOptions) => void;
  onBack: () => void;
}
...
const TimeAttackSelectionScreen: React.FC<TimeAttackSelectionScreenProps> = ({ onStartGame, onBack }) => {
  const { playClick } = useContext(SoundContext);

  const handleSelectDifficulty = (difficulty: Difficulty) => {
    playClick();
    onStartGame({ difficulty, duration: 60 });
  };
  ...
  return (
    <div className="w-full max-w-2xl mx-auto p-4 md:p-8 animate-fadeIn">
        ...
        <div className="bg-white dark:bg-gray-800 p-8 rounded-xl shadow-lg text-center">
            <h2 className="text-2xl font-bold text-slate-700 dark:text-gray-300 mb-4">Race the Clock!</h2>
            <p className="text-lg text-slate-600 dark:text-gray-400 mb-2">
                Start with 60 seconds. Each correct answer adds <span className="font-bold text-green-500">+2 seconds</span>.
            </p>
             <p className="text-lg text-slate-600 dark:text-gray-400 mb-6">
                But be careful! Each wrong answer costs you <span className="font-bold text-red-500">10 seconds</span> and resets your multiplier.
            </p>
            <p className="text-lg text-slate-600 dark:text-gray-400 mb-8 font-semibold">
...
export default TimeAttackSelectionScreen;
